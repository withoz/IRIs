...
module IRIS
  module Enscape
    ...
    # 축 하나로 만든 기저라 기구의 회전을 모릅니다. 그래서 한동안 방위
    # 평균으로 접어 보냈습니다(1차원). 비대칭 배광을 임의 각도로 돌려 그리는
    # 것보다 나았기 때문입니다.
    #
    # 이제 기구의 회전(롤)을 실어 보냅니다 — 노드 변환의 row0(로컬 +X)가 IES
    # 가로각 0도 평면이고, 그것이 ONB 기저에서 몇 도에 있는지를
    # Base.Scalars 의 상위 16비트로 넘깁니다(구면 광원은 하위만 씁니다).
    # 셰이더가 그만큼 빼서 맞춥니다. 그래서 2차원으로 되돌립니다.
    #
    # ⚠ **폭이 세로각, 높이가 가로각**입니다. 뒤집으면 배광이 90도 돌아간 채
    # 그럴듯하게 나옵니다.
    IES_NV = 128
    IES_NH = 64
    ...
    class << self
      ...
      def ies_grid(vert, horz, cand, scale, nv, nh, peak)
        ...
        IES_NH.times do |hi|
          # 셰이더: v = atan2(y, x) / (2pi) + 0.5  -> v=0 이 -180도입니다.
